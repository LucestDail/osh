...
## 운영 상태 (2026-09-25) — 배포 완료

홈랩 `.25` 라이브 대조 후 같은 날 배포까지 마쳤다.

- **라이브 `993fdb2`(09-14) → `0a7d395` 배포 완료.** 테스트 108개 통과, 재기동 후 21초에 200.
  뒤처져 있던 6커밋은 문서 정정이 대부분이고 기능은 `28cd1e3`(뉴스 SSE 키워드 필터) 하나였다.
  그 커밋이 "이 스트림을 부르는 클라이언트가 아직 없다"고 밝히고 있어 **결함이 아니라 미배포 기능**이었다.
- 롤백 jar: `target/osh-0.0.1-SNAPSHOT.jar.bak-20260925-110008`
- 포트 **9080**, context-path `/osh`. 외부는 `oshhome.duckdns.org:8030/osh/`.

### 배포 방법 (저장소 = 배포 위치)
서버는 `git pull` 이 안 된다(HTTPS 인증). **bundle** 로 옮긴다:
```bash
git bundle create /tmp/osh.bundle <서버SHA>..main
scp -O /tmp/osh.bundle <서버>:/tmp/
ssh <서버> 'cd ~/Workspace/osh && git fetch /tmp/osh.bundle main && git merge --ff-only FETCH_HEAD'
# jar 은 맥에서 빌드해 보낸다 (서버는 2013년 i5·8GB 라 빌드하지 않는다)
sudo systemctl restart osh
curl -s -o /dev/null -w '%{http_code}\n' http://127.0.0.1:9080/osh/   # 부팅 20~35초
```
